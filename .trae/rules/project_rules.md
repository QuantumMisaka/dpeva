...
DPEVA (Deep Potential EVolution Accelerator)，中文称“深度势能进化加速器”，是一个基于Python的，面向DPA3高效微调的主动学习项目。这个主动学习框架的核心是基于多模型不确定度和基于上游模型描述符（编码器）的高代表性数据采样的数据筛选。通过筛选出有价值的数据样本，DPEVA能够高效选取合适数据进行迭代微调，以高数据利用率获取特定化学空间通用的领域微调DPA3势。
