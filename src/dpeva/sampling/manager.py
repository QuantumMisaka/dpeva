...
class SamplingManager:
    ...
    def _run_direct(self, features, background_features=None):
        ...
        if self.n_candidates is not None:
            n_training_selected = sum(1 for idx in selected_indices if idx >= self.n_candidates)
            if n_training_selected > 0:
                self.logger.info(f"Joint Sampling: Selected {n_training_selected} training samples (kept for visualization).")
            # We no longer filter out training data here to allow visualization of full coverage.
