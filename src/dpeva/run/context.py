...
class RunContext:
    ...
    def _force_existing(
        ...
        try:
            ...
            fresh.record_event(kind="force", attempt_id=previous_attempt)
        except BaseException:
            # The old manifest remains archived and is never removed.  Any
            # partially written replacement is handled by atomic writers.
            raise
    ...
    def register_verified_artifacts(self, kind: str, paths: Sequence[Path]) -> None:
        ...
        for record in records:
            self.recorder.add_artifact(record)
...
def _atomic_json_write(path: Path, value: dict[str, Any]) -> None:
    ...
    try:
        payload = json.dumps(value, ensure_ascii=False, indent=2, default=str, allow_nan=False)
