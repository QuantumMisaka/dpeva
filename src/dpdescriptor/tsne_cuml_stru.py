...
# load data
desc_string = 'desc-v2-2-radsp/*/desc.npy'
dpdata_name = "FeCHO-clean-v2-2-28830"
dpdata_path = "./data-clean-v2-2-28830"
dpdata_string = "O*"

# t-SNE setting
perplex_num = 50
n_iter = 1600
save_name = f"tsne-desc-stru-{perplex_num}per-{n_iter}niter"
...
if os.path.exists(f'{save_name}.pickle'):
    print(f"{save_name}.pickle already exists, skip t-SNE.")
    ...
    # do t-SNE, most time consuming step
    gdf = cudf.DataFrame(all_desc_stru)

    print("Doing t-SNE...")
    tsne = cuml.TSNE(
        n_components=2,
        random_state=42,
        perplexity=perplex_num,
        n_iter=n_iter,
        init='pca',
    )

    embedding = tsne.fit_transform(gdf)
    embedding_np = embedding.to_pandas().values
    print("t-SNE done.")
...
plt.title(f't-SNE of {dpdata_name} dataset stru by perplexity {perplex_num} and niter {n_iter}')
