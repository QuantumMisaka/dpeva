...
## Test Data Verification (`test/fp-setting`) [PARTIAL]
- [ ] Generated `INPUT` matches reference in `abacus-inputs` (ignoring trivial formatting differences).
- [ ] Generated `STRU` matches reference (lattice, positions, magnetism).
- [ ] Processed output statistics match reference in `conveged_data_view`.
