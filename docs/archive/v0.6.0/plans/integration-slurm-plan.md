...
本计划面向 DP-EVA 的生产级工作流（Train / Infer / Feature / Collect / Analysis），基于项目在真实问题上的运行产物目录 `/test/test-for-multiple-datapool` (File missing) 反推输入输出边界，并交付可在 Slurm 队列上运行的集成测试编排与验收准则。
